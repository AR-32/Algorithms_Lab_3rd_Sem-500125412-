...
int main() {
    ...
    for (int i = 0; i < num_sizes; i++) {
        ...
        int** C_traditional = allocateMatrix(n);
        int** C_strassen = allocateMatrix(n);
        ...
        double traditionalTime = measureExecutionTime(traditionalMultiply, A, B, C_traditional, n);
        double strassenTime = measureExecutionTime(strassenMultiply, A, B, C_strassen, n);
        ...
        for (int j = 0; j < n; j++) {
            for (int k = 0; k < n; k++) {
                if (C_traditional[j][k] != C_strassen[j][k]) {
                    equal = 0;
                    break;
                }
            }
        ...
        if (equal) {
            printf("Results match for matrix size %d x %d\n", n, n);
        } else {
            printf("Results do not match for matrix size %d x %d\n", n, n);
        }

        freeMatrix(A, n);
        freeMatrix(B, n);
        freeMatrix(C_traditional, n);
